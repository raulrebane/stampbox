...
if ($customermailboxes) {
    while ($custmailbox = pg_fetch_assoc($customermailboxes)) 
        ...
        if ($mailboxconfig) {
            ...
            if ($mailconf['incoming_auth'] == 'USERNAME') {
                list($username, ) = explode("@", $custmailbox['e_mail_username']);
            }
            else {
                $username = $custmailbox['e_mail_username'];
            }
            echo 'going to try mailbox open with: ' ."{".$mailconf['incoming_hostname'] .":" .$mailconf['incoming_port'] ."/ssl/novalidate-cert}INBOX",
                 ' username: ' .$username .' pass: '. $custmailbox['e_mail_password'] ."\n";
            $inbox = imap_open("{".$mailconf['incoming_hostname'] .":" .$mailconf['incoming_port'] ."/ssl/novalidate-cert}INBOX",
                    $username,$custmailbox['e_mail_password']);
//	    syslog(LOG_INFO, "Customer: " .$custmailbox['customer_id'] ." - Started processing mailbox" .$mailconf['e-mail']);
